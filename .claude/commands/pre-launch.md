...
1. **architect** — Run `npm run typecheck`, check for outdated/conflicting dependencies (`npm outdated`), circular dependencies, dead code detection, duplicate code patterns.

2. **qa-lead** — Run `npm run test && npm run typecheck && npm run lint`. Report total test count, pass rate, failures. Check coverage for critical paths. Verify graceful degradation. Identify high-risk untested files.

3. **security-reviewer** — Run `npm audit`. Search for hardcoded secrets (API keys, tokens, passwords). Verify auth implementation. Check for injection vectors (XSS, SQL). Verify no secrets in client-visible env vars. Check CORS configuration. Check dependency licenses.

4. **performance-eng** — Run `npm run build` and parse output for bundle/artifact sizes. Flag oversized artifacts. Check for unused exports bloating the bundle. Assess code splitting. Check for performance anti-patterns.
...
- **Do NOT auto-fix during the audit.** Present the full audit to the user.
